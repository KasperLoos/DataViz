...
def _(mo, os, pd):
    ...
    pigs = pd.read_excel("Occurences.xlsx")
    ...
    return pigs, station_dropdown, stations


@app.cell
def _(mo, pigs, station_dropdown):
    ...
    pig_ids = [str(pig_id) for pig_id in pig_ids]
    pig_options = ["All"] + pig_ids
    ...
    return pig_dropdown, pig_ids, pig_options, selected_station, station_pigs
...
def _(pig_ids):
    ...
    id_to_coord = dict(zip([str(pig_id) for pig_id in pig_ids], coordinates_circles))
    return coordinates_circles, id_to_coord
...
@app.cell
def _(Line, Title, id_to_coord, pig_dropdown, pigs, station_pigs):
    lines = []
    ...
    if selected_pig == "All":
        ...
        pig_interactions = station_pigs[
            (station_pigs['Pig_1'].astype(str) == selected_pig) | 
            (station_pigs['Pig_2'].astype(str) == selected_pig)
        ]

        for _, row in pig_interactions.iterrows():
            pig1 = str(row['Pig_1'])
            pig2 = str(row['Pig_2'])
            opacity = row['Count']**2 / 250000
            ...
            if pig1 in id_to_coord and pig2 in id_to_coord:
                ...
                lines.append(
                    Line(
                        ...
                        elements=[Title(elements=[str(row['Count'])])],
                        stroke_opacity=opacity,
                        stroke="red" if selected_pig in (pig1, pig2) else "black"
                    )
                )
    return (
        lines,
...
def _(
    ...
    if selected_pig == "All":
        ...
        if selected_pig in id_to_coord:
            idx = pig_ids.index(selected_pig)
            x, y = coordinates_circles[idx]
            angle = [-0, -30, 30, 60, -60, 90, -90, 120, -120, 180, -180][idx]
        
            count = self_counts.get(selected_pig, 0)
            _opacity = (count / 2000) if count else 0.0
    ...
    return angle, count, idx, paths, pig_id, self_counts, x, y
...
@app.cell
def _(mo, pig_dropdown, plot, station_dropdown):
    mo.hstack([mo.Html(plot.as_str()), pig_dropdown, station_dropdown], justify= "start")
    return
